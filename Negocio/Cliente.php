...
function findAll() {
	 ...
	 */
	$fields = array('nombre','apellidos','CI','organizacion','direccion','telefono');

	$AND_CI = "";
	if (@$_REQUEST['filter'] != "") {
		$AND_CI = "AND c.CI LIKE " . GetSQLValueStringForSelect(@$_REQUEST["filter"], $filter_type);	
	}

      $CAMPOS_SELECCIONADOS = " p.nombre,p.apellidos,c.CI,c.organizacion,c.direccion,c.telefono"; 
	
	  $WHERE_AND = "where c.CI = p.CI";

      $FROM = "FROM cliente c, persona p";
	...
	$query_recordset = "SELECT $CAMPOS_SELECCIONADOS $FROM $WHERE_AND $AND_CI $order";
	//$query_recordset = "SELECT CI,organizacion,direccion,telefono FROM `cliente` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `cliente` (CI,organizacion,direccion,telefono) VALUES (%s,%s,%s,%s)" ,			GetSQLValueString($_REQUEST["CI"], "text"), # 
			GetSQLValueString($_REQUEST["organizacion"], "text"), # 
			GetSQLValueString($_REQUEST["direccion"], "text"), # 
			GetSQLValueString($_REQUEST["telefono"], "int")# 
	);
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `cliente` WHERE CI = %s",
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `cliente` SET organizacion = %s,direccion = %s,telefono = %s WHERE CI = %s", 
			GetSQLValueString($_REQUEST["organizacion"], "text"), 
			GetSQLValueString($_REQUEST["direccion"], "text"), 
			GetSQLValueString($_REQUEST["telefono"], "int"), 
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `cliente` WHERE CI = %s", 
			GetSQLValueString($row_recordset["CI"], "text")
		);
