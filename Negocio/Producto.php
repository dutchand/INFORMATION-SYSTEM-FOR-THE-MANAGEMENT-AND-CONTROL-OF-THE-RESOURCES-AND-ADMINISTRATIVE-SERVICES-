...
function findAll() {
	 ...
	 */
	$fields = array('Id_producto','unidad_medida','preciocosto','moneda');
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `producto` $where"); 
	...
	$query_recordset = "SELECT Id_producto,unidad_medida,preciocosto,moneda FROM `producto` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `producto` (Id_producto,unidad_medida,preciocosto,moneda) VALUES (%s,%s,%s,%s)" ,	GetSQLValueString($_REQUEST["Id_producto"], "text"), # 
			GetSQLValueString($_REQUEST["unidad_medida"], "text"), # 
			GetSQLValueString($_REQUEST["preciocosto"], "text"), # 
			GetSQLValueString($_REQUEST["moneda"], "text")# 
	);
	$ok = mysql_query($query_insert);
	
	$almacen->agregar_producto_almacen($_REQUEST["Id_producto"],$_REQUEST["cantidad_producto"],$_REQUEST["fecha_entrada"],$conn);
		...
		$toret = array(
			"data" => array(
				array(
					...
					"preciocosto" => $_REQUEST["preciocosto"], # 
					"moneda" => $_REQUEST["moneda"] # 
					//"cantidad_producto" => $_REQUEST["cantidad_producto"] #
					//"fecha_entrada" => $_REQUEST["fecha_entrada"]     #$_REQUEST["Id_almacen"],
				)
			), 
			"metadata" => array()
		);
		
		$auditoria->agregar_a_auditoria($_REQUEST["username"],"Agregó el nuevo producto " .$_REQUEST["Id_producto"]." en el almacen.","0");
...
/****************************************************************
 * modificar producto en el almacen
 * 
 */
...
function Modificar_p() {	
 ...
 $auditoria = new TAuditoria;
	  ...
	  $query_recordset = sprintf("SELECT Id_producto FROM `producto` WHERE Id_producto = %s ", 
	...
	if ($num_rows == 1)		
	 {  		
	$ok=$almacen->modificar_producto_almacen($_REQUEST["Id_producto"],$_REQUEST["new_cant"],$_REQUEST["fecha_modify"],$conn);
	 }	
	 		
		if ($ok) {
			// return the updated entry
			$toret = array(
				"data" => array("error" => "Fué modificado con éxito :" .$_REQUEST["Id_producto"]), 
				"metadata" => array()
			);
		//Registrar la creación del  usuario por el administrador en Auditoria	
	$auditoria->agregar_a_auditoria($_REQUEST["username"],"Modificó la cantidad del producto " .$_REQUEST["Id_producto"]." en el almacen.","0");
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `producto` WHERE Id_producto = %s",
		GetSQLValueString($_REQUEST["Id_producto"], "text")
	);
	$recordset = mysql_query($query_recordset, $conn);
	$num_rows = mysql_num_rows($recordset);
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `producto` SET unidad_medida = %s,preciocosto = %s,moneda = %s WHERE Id_producto = %s", 
			GetSQLValueString($_REQUEST["unidad_medida"], "text"), 
			GetSQLValueString($_REQUEST["preciocosto"], "int"), 
			GetSQLValueString($_REQUEST["moneda"], "text"), 
			GetSQLValueString($row_recordset["Id_producto"], "text")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => array(
					array(
						"Id_producto" => $row_recordset["Id_producto"], 
						"unidad_medida" => $_REQUEST["unidad_medida"], #
						"preciocosto" => $_REQUEST["preciocosto"], #
						"moneda" => $_REQUEST["moneda"]#
					)
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `producto` WHERE Id_producto = %s", 
			GetSQLValueString($row_recordset["Id_producto"], "text")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => $row_recordset["Id_producto"], 
				"metadata" => array()
			);
 $auditoria->agregar_a_auditoria($_REQUEST["username"],"Eliminó el producto " .$_REQUEST["Id_producto"]." del almacen.","0");
