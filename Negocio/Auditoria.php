...
function findAll() {
	 ...
	 */
	$fields = array('Id_auditoria','Id_usuario','fecha','accion_ejecutada');
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `auditoria` $where"); 
	...
	$query_recordset = "SELECT Id_auditoria,Id_usuario,fecha,accion_ejecutada FROM `auditoria` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `auditoria` (Id_usuario,fecha,accion_ejecutada) VALUES (%s,%s,%s)" ,			GetSQLValueString($_REQUEST["Id_usuario"], "text"), # 
			GetSQLValueString($_REQUEST["fecha"], "text"), # 
			GetSQLValueString($_REQUEST["accion_ejecutada"], "text")# 
	);
	...
	if ($ok) {
		...
		$toret = array(
			"data" => array(
				array(
					"Id_auditoria" => mysql_insert_id(), 
					"Id_usuario" => $_REQUEST["Id_usuario"], # 
					"fecha" => $_REQUEST["fecha"], # 
					"accion_ejecutada" => $_REQUEST["accion_ejecutada"]# 
				)
			), 
			"metadata" => array()
		);
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `auditoria` WHERE Id_auditoria = %s",
		GetSQLValueString($_REQUEST["Id_auditoria"], "int")
	);
	$recordset = mysql_query($query_recordset, $conn);
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `auditoria` SET Id_usuario = %s,fecha = %s,accion_ejecutada = %s WHERE Id_auditoria = %s", 
			GetSQLValueString($_REQUEST["Id_usuario"], "text"), 
			GetSQLValueString($_REQUEST["fecha"], "text"), 
			GetSQLValueString($_REQUEST["accion_ejecutada"], "text"), 
			GetSQLValueString($row_recordset["Id_auditoria"], "int")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => array(
					array(
						"Id_auditoria" => $row_recordset["Id_auditoria"], 
						"Id_usuario" => $_REQUEST["Id_usuario"], #
						"fecha" => $_REQUEST["fecha"], #
						"accion_ejecutada" => $_REQUEST["accion_ejecutada"]#
					)
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `auditoria` WHERE Id_auditoria = %s", 
			GetSQLValueString($row_recordset["Id_auditoria"], "int")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => $row_recordset["Id_auditoria"], 
				"metadata" => array()
			);
		} else {
			$toret = array(
